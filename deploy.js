...
if (!process.env.FTP_DEPLOY_USERNAME)
  throw new Error("FTP_DEPLOY_USERNAME not set");
if (!process.env.FTP_DEPLOY_PASSWORD)
  throw new Error("FTP_DEPLOY_PASSWORD not set");
...
function cleanRemote() {
  ...
  return sftp.list(remotePathBase).then(objectList => {
    ...
    let operations = directoriesToRemove
      ...
      .concat(
        filesToRemove.map(file =>
          sftp
            .delete(upath.join(remotePathBase, file))
...
function uploadFiles(items) {
  ...
  return Promise.all(
    filesToUpload.map(file =>
      sftp
        .put(file.localPath, file.remotePath)
